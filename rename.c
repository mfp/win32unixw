...
CAMLprim value unix_renameW(value path1, value path2)
{
  ...
  if (supports_MoveFileEx > 0)
    ok = MoveFileExW((LPCWSTR)String_val(path1), (LPCWSTR)String_val(path2),
                    MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH |
                    MOVEFILE_COPY_ALLOWED);
  else
    ok = MoveFileW((LPCWSTR)String_val(path1), (LPCWSTR)String_val(path2));
  if (! ok) {
